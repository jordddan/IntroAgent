...
if __name__ == "__main__":
    ...
    lst = [(1,2),(2,3),(3,4),(4,5)]
    for item in lst:
        intro_agent = IclAgnet(prompt_path=f"openfile/prompt.txt",dataset=dataset,batch_size=item[0],epoch=item[1])
        print(intro_agent.ref)
        step = intro_agent.batch_size
        intro_agent.test_all(f"openfile/icl/step{step}/intro_origin",600)
        # intro_agent.write_intro("intro_origin.txt",600)
        intro_agent.train()
        # intro_agent.write_intro("intro_trained.txt",600)
        intro_agent.test_all(f"openfile/icl/step{step}/intro_traianed",600)
